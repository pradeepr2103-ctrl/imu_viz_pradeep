...
static Mat4 ai2mat(const aiMatrix4x4& a) {
    // aiMatrix4x4 is row-major; Mat4 is column-major (OpenGL)
    Mat4 m;
...
bool Model::loadFromFile(const std::string& path) {
    Assimp::Importer imp;
    // Triangulate, gen normals, limit to 4 weights per vertex
    ...
    std::cout << "Model loaded: " << vertices.size() << " verts, "
              << indices.size()/3 << " tris, "
              << bones.size() << " bones" << std::endl;
    std::cout << "BBox Y: " << bboxMin.y << " .. " << bboxMax.y << std::endl;
    return true;
}

// Collect ALL bones across all meshes in skin-joint order
...
// Walk aiNode tree to set parent indices
void Model::buildParentIndices(const aiNode* node, int parentBoneIdx) {
    ...
    if(it != boneNameToIdx_.end()){
        ...
        // Store local node transform as the bind pose local TRS
...
void Model::uploadToGPU(){
    ...
    auto stride = (GLsizei)sizeof(SkinnedVertex);
    // location 0: position
