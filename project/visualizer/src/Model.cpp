...
bool Model::loadFromFile(const std::string& path) {
    ...
    // Store index count AFTER all meshes are processed
    indexCount_ = (unsigned int)indices.size();

    // Bounding box in raw model space (cm for Mixamo GLB)
    ...
    std::cout << "BBox Y (raw cm): " << bboxMin.y << " .. " << bboxMax.y << std::endl;
    std::cout << "BBox Y (scaled): " << bboxMin.y*0.01f << " .. " << bboxMax.y*0.01f << std::endl;
    return true;
}
...
void Model::uploadToGPU(){
    ...
    glBindVertexArray(0);
    std::cout << "GPU upload done (VAO=" << vao << ", indices=" << indexCount_ << ")" << std::endl;
}

void Model::draw() const {
    if(!indexCount_){ std::cerr << "draw(): indexCount_=0!\n"; return; }
    glBindVertexArray(vao);
